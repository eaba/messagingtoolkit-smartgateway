...
namespace MessagingToolkit.SmartGateway.Core
{
    ...
    public partial class frmServiceControl : Form
    {
        ...
        /// Service controller
        /// </summary>
        private ServiceController serviceControler;
        ...
        private void frmServiceControl_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                this.serviceControler = new ServiceController(ServiceParameter.WindowsServiceName, Environment.MachineName);
                txtStatus.Text = this.serviceControler.Status.ToString();
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            {
                // Get latest status
                if (this.serviceControler.Status == ServiceControllerStatus.Running)
                {
                    txtStatus.Text = Resources.ServiceAttemptStop;
                    txtStatus.Refresh();

                    this.serviceControler.Stop();
                    
                    // Wait for 1 minute for the service to stop
                    this.serviceControler.WaitForStatus(ServiceControllerStatus.Stopped, new TimeSpan(0, 1, 0));
                }

                txtStatus.Text = Resources.ServiceAttempStart;
                txtStatus.Refresh();

                this.serviceControler.Start();

                // Wait for 1 minute for the service to start
                this.serviceControler.WaitForStatus(ServiceControllerStatus.Running, new TimeSpan(0, 1, 0));
                txtStatus.Text = this.serviceControler.Status.ToString();
            }
            catch (Exception ex)
            {
        ...
        private void btnStop_Click(object sender, EventArgs e)
        {
            ...
            {
                // Get latest status
                if (this.serviceControler.Status == ServiceControllerStatus.Stopped)
                {
                    txtStatus.Text = this.serviceControler.Status.ToString();                  
